#NSForest.py(copied from github)
#modifications: add seeds.
import numpy as np
import pandas as pd
import scanpy as sc
...
def myRandomForest(adata, df_dummies, cl, n_trees, n_jobs, n_top_genes, seed):
#     x_train = adata.X
    x_train = adata.to_df()
    y_train = df_dummies[cl]
    rf_clf = RandomForestClassifier(n_estimators=n_trees, n_jobs=n_jobs, random_state=seed) #<===== criterion=“gini”, by default
...
def NSForest(adata, cluster_header, seed, cluster_list=None, medians_header=None,
    ...
    for cl in cluster_list:
        ...
        top_rf_genes = myRandomForest(adata, df_dummies, cl, n_trees, n_jobs, n_top_genes, seed)
